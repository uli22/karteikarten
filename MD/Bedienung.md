...
## 📁 Dateinamen-Korrektur

Wenn Karteikarten-Dateien umbenannt oder in andere Ordner verschoben wurden
(z.B. von `Gb` nach `Sb`), muessen die Pfade in der Datenbank angepasst werden.
Dafuer gibt es das Skript `korrigieren_dateinamen.py`.

### Ablauf

1. **Korrekturen.csv** im `input/`-Ordner mit den alten und neuen Pfaden befuellen
2. Blindlauf starten: `uv run korrigieren_dateinamen.py`
3. Ergebnis pruefen (Anzahl UPDATEs / Uebersprungene)
4. Bei korrektem Ergebnis ausfuehren: `uv run korrigieren_dateinamen.py --apply`

Das Skript aktualisiert `dateiname` und `dateipfad` in der Datenbank und erstellt
automatisch Sync-Queue-Eintraege fuer die Online-Synchronisation.

Siehe [Korrektur_Datei.md](Korrektur_Datei.md) fuer Details.
